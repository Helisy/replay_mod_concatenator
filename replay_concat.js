...
const camera_data_path = "./camera_data.txt";
const output_settings = "./output_settings.txt";
...
async function main(){
    ...
    const run_dir = settings.run_dir;
    ...
    for (values of Object.values(replay_data)) {
        ...
        await fs.writeFile(path.join(output_dir, `${values.name}.txt`), JSON.stringify(output_data));
        ...
        render_settings.settings.outputFile = path.join(output_render_dir, `video_${index}.mp4`);
        ...
        await fs.copyFile(path.join(tmp_dir, values.name + ".zip"), path.join(output_dir, values.name + ".mcpr"));  
    }   
}
